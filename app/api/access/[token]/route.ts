...
export async function GET(req: Request, { params }: { params: Promise<{ token: string }> }) {
  ...
  if (!redis) return new Response(null, { status: 302, headers: { Location: `${appUrl}/app` } })

  const email = await redis.get(`access:${token}`)
  if (!email) return new Response(null, { status: 302, headers: { Location: `${appUrl}/app` } })
  ...
  return new Response(null, {
    ...
    headers: {
      Location: `${appUrl}/app`,
      'Set-Cookie': `i2l_session=${sessionToken}; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=${60 * 60 * 24 * 30}`,
    },
  })
}
